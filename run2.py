...
def uniformly_sampled_input_experiment(d, n, m, p_set, sample_size):
    ...
    p1_min_arr = []
    for hash_function in hash_functions:
        p1_min_arr.append(min([hash_function[p1[idx]] for idx in range(n)]))
    dict_count = Counter(p1_min_arr)
    expected_real_mean = 0.0
    for x, y in dict_count.items():
        if d-x-1 >= n-1:
            result = n/d * p_set
            for idx in range(0, x):
                result *= (d-n-idx)/(d-1-idx)
            expected_real_mean += y * result

    print("p1_min_arr:", p1_min_arr)
    print("Average of p1_min_arr:", p1_min_arr)
    print("Expected real mean:", expected_real_mean)
